...
def parse_args(args):
    ...
    parser.add_argument("--dataset_trust_remote_code", default=False, type=lambda x: x.lower() == "true")
...
def load_model(args, cache_dir):
    ...
    def use_custom_llama_impl(config):
        return config.model_type == "llama" and not args.use_hf_model

    if args.model_config is not None:
        model_config = AutoConfig.from_pretrained(args.model_config, **get_pretrained_kwargs())
        if use_custom_llama_impl(model_config):
            model = LlamaForCausalLM(model_config)
        else:
            model = AutoModelForCausalLM.from_config(model_config, **get_pretrained_kwargs())
    elif args.model_name is not None:
        model_config = AutoConfig.from_pretrained(args.model_name, **get_pretrained_kwargs())
        if use_custom_llama_impl(model_config):
...
def resolve_tokenizer_name(args):
    ...
    if args.model_config is not None:
        ...
        if config_name_or_path and config_name_or_path != args.model_config and not config_name_or_path.endswith(".json"):
            return config_name_or_path

        if os.path.isfile(args.model_config):
            with open(args.model_config) as f:
                raw_config = json.load(f)
            raw_name_or_path = raw_config.get("_name_or_path")
            if raw_name_or_path:
                return raw_name_or_path

        if getattr(model_config, "name_or_path", None) and model_config.name_or_path != args.model_config:
...
def get_dataset_kwargs(args):
    ...
    if args.dataset_config_name is not None:
        dataset_kwargs["name"] = args.dataset_config_name
    if args.dataset_trust_remote_code:
        dataset_kwargs["trust_remote_code"] = True
    return dataset_kwargs

def load_dataset_for_lm(args):
    dataset_kwargs = get_dataset_kwargs(args)

    def ensure_text_field(dataset):
        if args.dataset_text_field == "text":
            return dataset
        return dataset.map(lambda x: {"text": x[args.dataset_text_field]})
    ...
    if os.path.exists(args.dataset_name):
        ...
        try:
            eval_dataset = datasets.load_dataset(args.dataset_name, split=f"validation[:{args.validation_split_percentage}%]")
        except (ValueError, KeyError):
            eval_dataset = datasets.load_dataset(args.dataset_name, split=f"train[:{args.validation_split_percentage}%]")
    else:
        data = datasets.load_dataset(args.dataset_name, split="train", streaming=True, **dataset_kwargs)
        try:
            split_names = datasets.get_dataset_split_names(args.dataset_name, **dataset_kwargs)
        except Exception:
            split_names = []
        eval_split = f"validation[:{args.validation_split_percentage}%]" if "validation" in split_names else f"train[:{args.validation_split_percentage}%]"
        eval_dataset = datasets.load_dataset(args.dataset_name, split=eval_split, **dataset_kwargs)

    data = ensure_text_field(data)
    eval_dataset = ensure_text_field(eval_dataset)
    return data, eval_dataset
...
def evaluate_model(model, preprocess_batched, pad_idx, global_rank, world_size, device, batch_size, dataset=None, is_icelandic_dataset=False):
    ...
    if remove_columns is None:
        remove_columns = ["text", "timestamp", "url", "prefix", "source", "target", "origin"]
...
def main(args):    
    ...
    data, eval_dataset = load_dataset_for_lm(args)
    ...
    tokenizer = AutoTokenizer.from_pretrained(tokenizer_name, model_max_length=args.max_length)

...
def ensure_text_field(dataset, text_field):
    if text_field == "text":
        return dataset
    return dataset.map(lambda x: {"text": x[text_field]})
...
def load_dataset_for_lm(args):
    ...
    if os.path.exists(args.dataset_name):
        ...
        return ensure_text_field(data, args.dataset_text_field), ensure_text_field(eval_dataset, args.dataset_text_field)
    ...
    if "validation" in split_names:
        eval_dataset = datasets.load_dataset(args.dataset_name, split="validation", streaming=True, **dataset_kwargs)
    else:
        logger.info("Dataset has no validation split; using an independent shuffled train stream for evaluation.")
        eval_dataset = datasets.load_dataset(args.dataset_name, split="train", streaming=True, **dataset_kwargs)
...
def main(args):    
    ...
    run_config.update({
        ...
        "dataset": args.dataset_name or "allenai/c4",
